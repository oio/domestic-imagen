...
class GenerateRequest(BaseModel):
	...
	height: int = 1024
	width: int = 1024
	# Add source field to identify request origin (e.g. "discord")
...
async def queue_processor():
	...
	while True:
		...
		req, background_tasks, response_queue, is_discord = request_tuple
		...
		try:
			...
			if is_discord:
				# For Discord requests, collect all the generation data and send as one complete response
				all_data = []
				async for chunk in event_stream_internal(req, background_tasks):
					if chunk.startswith("data: "):
						# Extract just the data from the SSE format
						data_content = chunk[6:].strip()
						# Only add JSON data (the final result) to all_data
						if data_content.startswith("{"):
							all_data.append(json.loads(data_content))
						else:
							# For debug, you might want to know about intermediate steps
							print(f"Discord process step: {data_content}")
				
				# Get the final result (should be the last item)
				if all_data:
					final_result = all_data[-1]
					# Put the complete result in the queue
					await response_queue.put(final_result)
				else:
					await response_queue.put({"error": "No data generated"})
			else:
				# For regular streaming requests, process each chunk as it comes
				async for chunk in event_stream_internal(req, background_tasks):
			...
			# Handle errors
			if is_discord:
				await response_queue.put({"error": f"Error during processing: {str(e)}"})
			else:
...
async def generate_image(req: GenerateRequest = Body(...), background_tasks: BackgroundTasks = None):
	...
	# For non-Discord requests, provide queue position updates
	if not is_discord and queue_position > 0:
	...
	await request_queue.put((req, background_tasks, response_queue, is_discord))
	print(f"Added request to queue at position {queue_position} (Discord: {is_discord})")
	
	# Handle differently based on source
	if is_discord:
		# For Discord: wait for the complete response, then return it as JSON
		final_result = {}
		while True:
			...
			# Store the result (overwriting any previous)
			final_result = result
		
		# Return the final result as a JSON response
		return JSONResponse(content=final_result)
	else:
		# For regular streaming response:
